...
public class ModbusInverterQueryStrategy extends ModbusQueryStrategy<InverterDataDTO, ModbusInverter> {
    ...
    @Override
    protected InverterDataDTO createResult(ModbusConfig.ConfigSection modbusConfig, TCPModbusClient client, Map<String, Double> calculatedValues, VariableProvider provider, int offset) throws Exception {

        double currentDcPowerW = evaluateEntry("current_dc_power", modbusConfig, client, calculatedValues, provider, offset);
        double currentDcVoltageV = evaluateEntry("current_dc_voltage", modbusConfig, client, calculatedValues, provider, offset);
        double currentAcPowerW = evaluateEntry("current_ac_power", modbusConfig, client, calculatedValues, provider, offset);
