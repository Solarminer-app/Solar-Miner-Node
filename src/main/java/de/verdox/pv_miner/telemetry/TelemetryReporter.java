...
public class TelemetryReporter {
    ...
    public TelemetryReporter(
        ...
        this.version = versionFrom(buildPropertiesProvider);
        this.locationGrid = locationGrid == null || locationGrid.isBlank() ? null : locationGrid.trim();
    }

    /**
     * The real build version from the Spring Boot build-info
     * ({@code META-INF/build-info.properties}, generated by the Gradle
     * {@code buildInfo()} task from {@code frontendVersion}) — no env var needed.
     * Falls back to "dev" when the bean is absent (e.g. bare test contexts).
     */
    private static String versionFrom(ObjectProvider<BuildProperties> buildPropertiesProvider) {
        BuildProperties props = buildPropertiesProvider == null ? null : buildPropertiesProvider.getIfAvailable();
        if (props != null && props.getVersion() != null && !props.getVersion().isBlank()) {
            return props.getVersion().trim();
        }
        return "dev";
    }
